...
BPF_ENRICHER_PATH := internal/pkg/daemon/enricher/source/bpf
BPF_ENRICHER_FILES := $(shell find internal/pkg/daemon/enricher/source/bpf -type f -name '*.c' -or -name '*.h')
BPF_ENRICHER_OUTPUT_FILES := $(shell find internal/pkg/daemon/enricher/source/bpf -type f -name 'enricher.bpf.o.*')
...
update-bpf: clean \
    ...
    internal/pkg/daemon/enricher/source/bpf/enricher.bpf.o.amd64 \
    internal/pkg/daemon/enricher/source/bpf/enricher.bpf.o.arm64
...
internal/pkg/daemon/enricher/source/bpf/enricher.bpf.o.%: $(BPF_ENRICHER_FILES) ## Build and update all generated BPF code with nix
	nix-build nix/default-bpf-$*.nix
	cp -f result/enricher.bpf.o ./internal/pkg/daemon/enricher/source/bpf/enricher.bpf.o.$*
	chmod 0644 ./internal/pkg/daemon/enricher/source/bpf/enricher.bpf.o.$*
